...
import java.util.*;
import java.sql.*;
>>>>>>> a9843711d67ec96fb47aa02295db5497cb26dcff
...
<<<<<<< HEAD
        ...
        assertNull("Requesting too many items did not return an empty item list", inventory.item);
    }
    ...
    public void testEvaluateRequest1() {
    	 Inventory inventory = new Inventory();
         // Create a user request
         ...
         // Try using testEntry as input check that there is no possible combination of items to fill request
         inventory.analyzeEntry(testEntry);
         String testEntry = "mesh chair, 3";
         inventory.pullData();
         inventory.evaluateRequest();
         ArrayList<String> got = inventory.item.IDs;
